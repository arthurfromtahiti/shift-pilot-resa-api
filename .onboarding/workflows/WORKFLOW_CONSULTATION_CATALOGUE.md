...
Permettre à un client web de récupérer la liste complète des transferts inter-îles — avec leur prix et leur nombre de places restantes — afin de les afficher à l'utilisateur. Ce flux couvre l'intégralité de la lecture du catalogue : un appel HTTP unique retourne toujours l'ensemble des transferts sans filtre, sans pagination, sans authentification. C'est la seule route de lecture du service.
...
4. `listTransfers()` est appelée et retourne le tableau `transfers` brut (les 3 transferts en mémoire, `src/transfers.js:9-11`).
5. Le tableau est projeté via `.map(t => ({ id: t.id, from: t.from, to: t.to, price: t.price, seatsLeft: seatsLeft(t) }))` — `seats` et `sold` sont **masqués** (`src/server.js:14-20`).
6. Pour chaque transfert, `seatsLeft(t) = t.seats - t.sold` est calculé (`src/transfers.js:13-15`).
7. `sendJson(res, 200, body)` sérialise en JSON et répond avec `Content-Type: application/json` et statut 200 (`src/server.js:5-8`).

## Règles métier

- **Catalogue complet, sans filtre** : `listTransfers()` retourne `transfers` tel quel, sans filtre ni tri (`src/transfers.js:9-11`). Il n'existe aucune route de filtrage.
- **Projection publique obligatoire** : les champs `seats` (capacité brute) et `sold` (places déjà vendues) sont **exclus** de la réponse ; seul `seatsLeft` (calculé) est exposé (`src/server.js:14-20`). Cela masque l'historique de remplissage.
...
- **Calcul `seatsLeft` à chaque appel** : `seatsLeft(t) = t.seats - t.sold` est recalculé dynamiquement ; si `bookSeats()` a muté `sold` entre-temps, la valeur reflète immédiatement l'état courant (`src/transfers.js:13-15`).
...
- `src/transfers.js:17-19` — `isFull()` (exporté mais non utilisé dans ce workflow)
